...
def serve_prediction_plot(
    ...
    if hasattr(model, "decision_function"):
        ...
        y_pred_train = (model.predict(X_train) > threshold).astype(int)
        y_pred_test = (model.predict(X_test) > threshold).astype(int)
...
def serve_roc_curve(model, X_test, y_test):
    if hasattr(model, "decision_function"):
        ...
        decision_test = model.predict(X_test)
...
def serve_pie_confusion_matrix(model, X_test, y_test, Z, threshold):
    ...
    if hasattr(model, "decision_function"):
        ...
        y_pred_test = (model.predict(X_test) > scaled_threshold).astype(int)
